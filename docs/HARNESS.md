...
An error code names the failure class, not just the layer: `invalid`, `protocol` and `operation` reject a malformed request; `not_found`, `retired`, `conflict`, `state`, `fenced`, `busy`, `in_progress`, `indeterminate_tool`, `indeterminate_model`, `stale_context`, `stale_plan`, `provenance_required` and `budget` describe a refused lifecycle step, of which `busy` and `conflict` are worth retrying once the competing writer or turn finishes; `closed`, `schema`, `integrity`, `storage_error` and `internal_error` report a connection or database that cannot serve the request as written. `integrity` means a stored value does not match the column type the engine depends on, and `internal_error` means the operation failed unexpectedly. Lock contention is `busy`, never `storage_error`: the two call for different handling, and only the first is retryable. Nothing retries automatically.

A panic inside one operation is contained at the bridge: the response is a coded `internal_error` carrying the panic text, the operation's transaction has already rolled back, and the process keeps serving later requests. The host holds one persistent channel per engine, so one malformed row must not end it. Treat `internal_error` as a defect to report, not a transient condition.
...
Journal schema 4 adds confirmed model responses, retired-ID tombstones, provenance edges and action leases. Opening a supported schema-1 journal upgrades it additively. Stop older workers during upgrades and retain a SQLite backup. Older binaries refuse new schema-4 connections; database triggers additionally abort both the insertion of a turn with a retired request ID and the renaming of a live turn onto one, so already-open older connections cannot reintroduce a retired ID through either path. Those guards cover `cmp_turns`; the child journal tables rely on foreign-key enforcement, which is per-connection. Schema 4 also declares `cmp_turns.request_id` `NOT NULL`: a `TEXT PRIMARY KEY` on a rowid table is nullable in SQLite, and a NULL request ID reached the retention scan. The version stays 4 because the constraint only removes states the schema already admitted, so a rebuilt database is still read correctly by an older binary and a version bump would turn a compatible tightening into a hard refusal. `Open` rebuilds a database written before the constraint and skips the rebuild once the column carries it; `info` reports `cmp_turns_request_id_not_null` to tell the two states apart. A database that already holds a NULL request ID is refused with `integrity` naming the rows, and repairing them is an explicit operator step. See [RETENTION.md](RETENTION.md) for the rebuild, the retention convergence and plan-hash contracts.
